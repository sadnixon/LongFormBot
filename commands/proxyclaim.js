...
async function execute(interaction, user) {
  ...
  if (
    ...
    gameState.refChain[gameState.refChain.indexOf(userId) + 1] in
      gameState.refClaims
  ) {
    return interaction.reply({
  ...
  const reffedIndex = gameState.players
    .map((e) => e.id)
    .indexOf(gameState.refChain[gameState.refChain.indexOf(userId) + 1]);
  if (
  ...
  gameState.refClaims[
    gameState.refChain[gameState.refChain.indexOf(userId) + 1]
  ] = targetClaim;
  ...
  await announceChannel.send(
    standardEmbed(
      ...
      `**<@${userId}> claims the Ref of the Rain has revealed that <@${gameState.refChain[gameState.refChain.indexOf(userId) + 1]}> is on the ${targetClaim.toUpperCase()} team!**`,
    ),
  );
