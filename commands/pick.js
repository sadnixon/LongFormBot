...
async function execute(interaction, user) {
  ...
  const pickedBefore =
    interaction.user.id in gameState.missionPicks[gameState.missionIndex];
  ...
  if (
    ...
    await pickChannel.send(
      `${currentPlayers.map((e) => `<@${e}>`).join(' ')}\nThe mission chosen by <@${interaction.user.id}> has passed!`,
    );
